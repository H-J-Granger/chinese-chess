...
std::vector<std::pair<int, int>> S[HEIGHT][WIDTH];
		...
		for (int j = 0; j < WIDTH; j++){
			...
			for (auto pos : S[i][j]){
				if(mp.data_color[i][j] == red) {
					Count_Red[pos.first][pos.second]++;
				}
				if(mp.data_color[i][j] == green) {
					Count_Green[pos.first][pos.second]++;
				}
			}
		}
	}
	...
	for (int i = 0; i < HEIGHT; i++){
		for (int j = 0; j < WIDTH; j++){
			...
			switch (mp.data_type[i][j]){
				...
				case zu: {
					val *= 35;
					if ((mp.data_color[i][j] == red && i <= HEIGHT / 2) || 
						(mp.data_color[i][j] == green && i > HEIGHT / 2)) {
						val *= 2;
					}
					break;
				}
			...
			if (mp.data_color[i][j] == red){
				...
				if (mp.data_type[i][j] != jiang && Count_Red[i][j]) {
					val *= 2;
					val = abs(val);
					if (mp.data_color[i][j]==green) {val *= -1;}
				}
			} else {
				if (Count_Red[i][j]) {val *= -1;}
				if (mp.data_type[i][j] != jiang && Count_Green[i][j]) {
					val *= 2;
					val = abs(val);
					if(mp.data_color[i][j]==green) {val *= -1;}
				}
			}
			if (mp.data_type[i][j] == jiang) {
				ans += val;
				continue;
			}
...
int Min_Max_Search(int dep = 0){
	...
	for (int i = 0; i < HEIGHT; i++) {
		for (int j = 0; j < WIDTH; j++) {
			if((dep & 1) ? map.data_color[i][j] != red : map.data_color[i][j] != green) {
				continue;
			}
